...
public class FavoriteDataControllerTest extends AbstractControllerTest {
    ...
    public void getFavoriteRangesFromKinvey_succeed() throws Exception {
        User user = login();
        getFavoriteRanges(user);
    }

    private List<FavoriteRange> getFavoriteRanges(User user) throws Exception {
        ...
        assertNull(chinaSpace.read(new FavoriteRange()));

        return favoriteRanges;
    }

    @Test
    public void getFavoriteRangesFromSpace_succeed() throws Exception {
        User user = login();

        List<FavoriteRange> favoriteRanges = getFavoriteRanges(user);

        HttpHeaders headers = createHeaders("Kinvey " + KINVEY_AUTHTOKEN);
        ResultActions result = mockMvc.perform(get(favoriteRangesPath).headers(headers));
        validateFavoriteRangesResponse(favoriteRanges, result);

        KinveyAsyncGetRequest asyncRequest = usSpace.read(new KinveyAsyncGetRequest());
        assertNotNull(asyncRequest);
        assertEquals(favoriteRangesPath, asyncRequest.getUrl());
        assertEquals(FavoriteRange.class, asyncRequest.getEntityType());
        assertEquals(headers, asyncRequest.getHeaders());
        assertEquals(Boolean.FALSE, asyncRequest.getSaveToSpace());
        assertEquals(Boolean.FALSE, asyncRequest.isReplicable());
    ...
    public void getFavoriteDocumentsFromKinvey_succeed() throws Exception {
        ...
        waitForEmptyReplicationBacklog();
        assertNull(chinaSpace.readIfExists(template));
    }
    ...
    public void getFavoriteProductsFromKinvey_succeed() throws Exception {
        ...
        waitForEmptyReplicationBacklog();
        assertNull(chinaSpace.readIfExists(new FavoriteProduct()));
    }
    ...
    private FavoriteDocument createFavoriteDocument(User user) {
        ...
        Map<String, Object> documentAttr = Collections.singletonMap("doc1", "doc1values");
